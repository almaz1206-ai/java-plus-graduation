...
public class EventErrorHandler {
    ...
    EventApiError notFound(NotFoundException e) { return error(e.getMessage(), "The required object was not found.", HttpStatus.NOT_FOUND, List.of()); }
    @ExceptionHandler(ConflictException.class) @ResponseStatus(HttpStatus.CONFLICT)
    EventApiError conflict(ConflictException e) { return error(e.getMessage(), "For the requested operation the conditions are not met.", HttpStatus.CONFLICT, List.of()); }
    @ExceptionHandler({BadRequestException.class, MethodArgumentNotValidException.class}) @ResponseStatus(HttpStatus.BAD_REQUEST)
    EventApiError badRequest(Exception e) { return error(e.getMessage(), "Incorrectly made request.", HttpStatus.BAD_REQUEST, List.of()); }
