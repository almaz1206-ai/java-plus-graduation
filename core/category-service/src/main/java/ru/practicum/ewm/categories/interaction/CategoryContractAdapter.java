...
public class CategoryContractAdapter implements CategoryContract {
    ...
    @Override public CategoryResponse getById(Long id) {
        return repository.findById(id).map(this::map)
                .orElseThrow(() -> new NotFoundException("Category with id=" + id + " was not found"));
    }
    @Override public CategoriesResponse getByIds(IdsRequest request) {
        return new CategoriesResponse(repository.findAllById(request.ids()).stream().map(this::map).toList());
    }
    private CategoryResponse map(Category category) { return new CategoryResponse(category.getId(), category.getName()); }
}
