...
public class RequestInternalController {
    ...
    public ConfirmedRequestCountResponse count(@PathVariable Long eventId) {
        return contract.getConfirmedCount(eventId);
    }

    @PostMapping("/events/confirmed-counts")
    public ConfirmedRequestCountsResponse counts(@RequestBody IdsRequest ids) {
        return contract.getConfirmedCounts(ids);
    }

    @GetMapping("/users/{userId}/events/{eventId}/exists")
    public ParticipationRequestExistsResponse exists(@PathVariable Long userId, @PathVariable Long eventId) {
